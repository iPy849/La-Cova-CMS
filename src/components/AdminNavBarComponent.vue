<template>
    ...
    <header class="bg-white border-b-2">
        <nav class="mx-auto flex max-w-7xl items-center justify-between flex-row-reverse lg:flex-row p-6 lg:px-8"
             aria-label="Global">
            <div class="flex lg:flex-1 hidden lg:block">
                <a href="#" class="-m-1.5 p-1.5 font-semibold bg-blue-500 text-white rounded-full px-4 py-2">
            ...
            <div class="hidden lg:flex lg:gap-x-12">
                <RouterLink v-for="(navItem, i) in navItems" :key="i"
                            class="text-sm font-semibold leading-6 active:underline"
                            :class="{ 'underline underline-offset-8 decoration-4 decoration-blue-500': navItem.route.name === route.name }"
                            :to="navItem.route">
                    <component v-bind:is="navItem.icon" class="h-6 w-6 mr-2 inline"></component>
                    {{ navItem.name }}
                </RouterLink>
            </div>
        ...
        <div class="lg:hidden" role="dialog" aria-modal="true" v-if="isMobileMenuOpen">
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <a href="#" class="-m-1.5 p-1.5 font-semibold text-sm">
                        Bienvenido {{ username }}
                    </a>
                            ...
                            <div class="-mx-3">
                                ...
                                <div class="mt-2 space-y-2" id="disclosure-1">
                                    <RouterLink v-for="(navItem, i) in navItems" :key="i"
                                                ...
                                                :class="{ 'bg-gray-100': navItem.route.name === route.name }"
                                                :to="navItem.route"
                                                @click="isMobileMenuOpen = false">
                                        <component v-bind:is="navItem.icon" class="h-6 w-6 mr-2 inline"></component>
                                        {{ navItem.name }}
...
import {PaintBrushIcon, PencilIcon, CurrencyDollarIcon, ChevronRightIcon} from "@heroicons/vue/20/solid/index.js";
...
const navItems = ref([
    {name: "Edición de menú", route: {name: 'menu-editor'}, icon: PencilIcon},
    {name: "Edición de diseño", route: {name: 'design-editor'}, icon: PaintBrushIcon},
    {name: "Edición de cambio de moneda", route: {name: 'currency-editor'}, icon: CurrencyDollarIcon},
]);
const isMobileMenuOpen = ref(false);
const isLogoutDialog = ref(false);
const adminStorage = useAdminSessionStore();
...
const username = computed(() => {
    try {
        return adminStorage.userData.name;
    } catch {
        return '';
    }
});
